...
try:
    ...
    class _DropNeoAraBERTLoadReport(_std_logging.Filter):
        def filter(self, record):
            return "LOAD REPORT" not in record.getMessage()

    _hf_logging.get_logger("transformers.modeling_utils").addFilter(_DropNeoAraBERTLoadReport())
except Exception:
    pass
...
class NeoAraBERTConfig(PretrainedConfig):
    ...
    # The defaults below are placeholders in the source template. The export
    # pipeline rewrites them to the actual yaml values when bundling the
    # trained checkpoint, so an exported model.py advertises the same numbers
    # as config.json.
    def __init__(
        self,
        hidden_size: int = 768,
...
class NeoAraBERT(NeoAraBERTPreTrainedModel):
    ...
    def __init__(self, config: NeoAraBERTConfig):
        ...
        freqs_cis = precompute_freqs_cis(config.hidden_size // config.num_attention_heads, config.max_position_embeddings)
...
class NeoAraBERTForSequenceClassification(NeoAraBERTPreTrainedModel):
    config_class = NeoAraBERTConfig
